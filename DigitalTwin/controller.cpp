...
int main(int argc, char *argv[]){
	...
	char localIP[] = {"127.0.0.1"}, name[32], ip[16];
	char buffer[BUFSIZ];
    unsigned char actuatorStatus[6][3] = {0};
	float floatsToSend[7] = {0}, floatsToReceive[7] = {0};
    int serverSocket, portno, n, check, maxLoop = 100, numFloats=7, numChars = 18;
...
// Initiate platform object
    ...
    CircularBuffer myCircularBuffer(numberOfElements, elementSize, elementType, 
	 fileName, formatSpecifier);

    charBuffer = (unsigned char*)malloc(numCharsToGet);
...
for(int i=0;i<maxLoop;i++){
    ...
    if(getFeedbackLengthFromTwin(serverSocket, floatsToReceive, actuatorStatus,charBuffer)){
		std::cout << "Error in getFeedbackLengthFromTwin in controller" << std::endl;
		return 1;	
	}
    for(int j=0;j<6;j++){
        feedBackLengths[j] = (double)floatsToReceive[j];

        cout << "actuator " << j << " status: " << actuatorStatus[j][0] << ", " <<  actuatorStatus[j][1] << ", " <<  actuatorStatus[j][2] << endl;
    }
